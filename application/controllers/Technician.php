...
class technician extends MY_Controller {
	...
	public function __construct()
	{
		...
		if ( ! $this->session->has_userdata('Access')
			OR ! in_array($this->session->userdata('Access')['Acc_ID'],array('1','3','4','5'))) // 1 Administrator, 3 Technical, 4 ,5
		{
			redirect();
		}
	...
	public function view_job_samsung()
	{
		$this->data['detail'] = $this->db->where('Pro_ID', '1')->where_in('sv_register.Status_ID', array('1','2','3','4'))->order_by('Regis_ID','DESC')
		...
		$this->data['body'] = $this->load->view('technician/table/table_view_job_samsung',$this->data,TRUE);
		$this->load->view('_layouts/blank',$this->data);
	}

	public function view_job_nonsamsung()
	{
		$this->data['detail'] = $this->db->where('Pro_ID', '2')->where_in('sv_register.Status_ID', array('1','2','3','4'))->order_by('Regis_ID','DESC')
		->join('sv_user', 'sv_register.User_ID = sv_user.User_ID', 'left')
		->join('sv_status', 'sv_register.Status_ID = sv_status.Status_ID')
		->get('sv_register')->result_array();

		$this->data['body'] = $this->load->view('technician/table/table_view_job_nonsamsung',$this->data,TRUE);
		$this->load->view('_layouts/blank',$this->data);
	}
	...
	public function edit_job($ref_no=FALSE)
	{
			...
			if ($post['Status_ID'] == '2')
			{
				if ($this->Technician_model->update_status($post, $ref_no))
				{
					if ($this->Technician_model->update_status_assign($post, $ref_no))
					{
						...
						redirect('technician/edit_job/'.$ref_no);
					}
				}
			}
			// Status Quotation
		...
		$this->data['body'] = $this->load->view('technician/form/edit_job',$this->data,TRUE);
		$this->load->view('_layouts/blank',$this->data);
	}
